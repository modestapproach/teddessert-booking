...
export const deleteHandler = async ({ ctx, input }: DeleteOptions) => {
  // no-Postgres fork (`ctx.user.uuid` = the dibslist authUserId): delete the Convex
  // (webhookEndpoints) row via the owner adapter instead of prisma.
  if (ctx.user.uuid) {
    const { deleteOwnerWebhook } = await import("@calcom/lib/server/calcomWebhookAdapters");
    await deleteOwnerWebhook({ ownerAuthUserId: ctx.user.uuid, id: input.id });
    return { id: input.id };
  }
