...
export const getUserTopBannersHandler = async ({ ctx }: Props) => {
  // no-Postgres fork: `prisma.credential`/membership reads below THROW (no DB), so on the
  // fork (`ctx.user.uuid` = the dibslist authUserId) return the all-empty banner object.
  if (ctx.user.uuid) {
    return {
      teamUpgradeBanner: [],
      orgUpgradeBanner: [],
