...
import type { BookingsProcedureContext as EditLocationCtx } from "./util";
...
export const bookingsRouter = router({
  ...
  editLocation: bookingsProcedure.input(ZEditLocationInputSchema).mutation(async ({ input, ctx }) => {
    ...
    // CV-9: the `bookingsProcedure` middleware now throws NOT_IMPLEMENTED before any
    // prisma (it no longer resolves `ctx.booking`), so this handler is unreachable —
    // but its (disabled) body still types against `BookingsProcedureContext`. The
    // cast bridges the now-booking-less ctx; the handler throws as its first
    // statement regardless.
    return editLocationHandler({
      ctx: ctx as typeof ctx & EditLocationCtx,
      input,
      actionSource: "WEBAPP",
    });
  }),
