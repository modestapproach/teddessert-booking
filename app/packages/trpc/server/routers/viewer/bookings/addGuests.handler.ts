...
export const addGuestsHandler = async ({
  ...
  // CV-8 — CLEANLY DISABLED (no post-create add-guest model in our Convex
  // backend; cal's flow re-syncs attendee/calendar references through the
  // attendee + credential services, none of which our backend models). The
  // original body began with `getBooking` (prisma.booking…) which THREW on the
  // no-Postgres fork. We return EARLY with a friendly TRPCError BEFORE any
  // ctx.prisma access. The helper exports below (getBooking/validateUserPermissions/
  // updateBookingAttendees/…) are RETAINED for type-compat — the platform
  // BookingAttendeesService re-imports them — but are never reached from this
  // handler (documented in CONVEX-REWIRE-NOTES §CV-8). zod schema + UI UNTOUCHED.
  throw new TRPCError({
    code: "NOT_IMPLEMENTED",
    message: "Adding guests to a booking is not available in this deployment.",
  });
