...
export async function editLocationHandler({ ctx, input, actionSource }: EditLocationOptions) {
  // CV-8 — CLEANLY DISABLED (no post-create location-edit + calendar/location
  // re-sync model in our Convex backend; cal's flow edits the live booking and
  // re-syncs the location through the EventManager + credential/calendar services,
  // none of which our backend models). The original body began with
  // `new UserRepository(prisma).findByIdOrThrow` which THREW on the no-Postgres
  // fork. We return EARLY with a friendly TRPCError BEFORE any ctx.prisma access.
  // The helper exports (getLocationForOrganizerDefaultConferencingAppInEvtFormat,
  // UserError/SystemError) stay for type-compat (documented in
  // CONVEX-REWIRE-NOTES §CV-8). zod schema + the React EditLocationDialog UNTOUCHED.
  throw new TRPCError({
