...
export const getHandler = async ({ ctx, input }: GetOptions) => {
  // CV-4 — REWIRED onto the dibslist Convex backend's owner-scoped `listBookings`
  // (scheduling/bookingAdmin). cal's Postgres/Prisma + Kysely union query
  // (`getAllUserBookings`/`getBookings`, retained below for types/reference) is
  // SKIPPED on this path. The owner is `ctx.user.uuid` (the trusted dibslist
  // authUserId the fork carries on the session); cal's int `user.id` is the calId.
  //
  // Support both offset-based (list) and cursor-based pagination (calendar) — the
  // cursor is just the offset as a string (fake cursor pagination), which the Convex
  // adapter maps onto its own cursor envelope.
  ...
  const { listBookingsViaConvex } = await import("@calcom/lib/server/convexBookingsListAdapter");
  const { bookings, recurringInfo, totalCount, nextCursor } = await listBookingsViaConvex({
    ownerAuthUserId: user.uuid,
    filters: {
      status: input.filters.status,
      statuses: input.filters.statuses,
      afterStartDate: input.filters.afterStartDate,
      beforeEndDate: input.filters.beforeEndDate,
    },
    take,
    skip,
  });
