...
import {
  ZGetInteractionModeSchema,
  ZUpdateInteractionModeSchema,
} from "./interactionMode.schema";
import { ZResolveCoHostByEmailInputSchema } from "./resolveCoHostByEmail.schema";
...
export const eventTypesRouter = router({
  ...
  // CV-7 — resolve a dibslist email → co-host candidate for the add-by-email
  // picker. Owner-scoped inside the handler (uses ctx.user.uuid); a plain
  // authedProcedure is the right gate since dibslist has no team/PBAC model.
  resolveCoHostByEmail: authedProcedure
    .input(ZResolveCoHostByEmailInputSchema)
    .query(async ({ ctx, input }) => {
      const { resolveCoHostByEmailHandler } = await import("./resolveCoHostByEmail.handler");

      return resolveCoHostByEmailHandler({
        ctx,
        input,
      });
    }),
  ...
  // (per-slot lottery). Ownership via the CV-9 Convex pbac middleware (input.id
  // is the cal int event-type id).
  getInteractionMode: createEventPbacProcedure("eventType.read", [
    MembershipRole.ADMIN,
    MembershipRole.OWNER,
  ])
    .input(ZGetInteractionModeSchema)
    .query(async ({ ctx, input }) => {
      const { getInteractionModeHandler } = await import("./interactionMode.handler");
      return getInteractionModeHandler({ ctx, input });
    }),

  updateInteractionMode: createEventPbacProcedure("eventType.update", [
    MembershipRole.ADMIN,
    MembershipRole.OWNER,
  ])
    .input(ZUpdateInteractionModeSchema)
    .mutation(async ({ ctx, input }) => {
      const { updateInteractionModeHandler } = await import("./interactionMode.handler");
      return updateInteractionModeHandler({ ctx, input });
    }),
});
