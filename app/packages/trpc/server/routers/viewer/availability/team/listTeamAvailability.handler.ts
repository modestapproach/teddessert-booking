...
export const listTeamAvailabilityHandler = async ({ ctx, input }: GetOptions) => {
  // no-Postgres fork: `prisma.membership`/schedule reads THROW (no DB), so on the fork
  // (`ctx.user.uuid` = the dibslist authUserId) return the empty team-availability table.
  if (ctx.user.uuid) {
    return {
      rows: [],
