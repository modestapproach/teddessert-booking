...
export const changePasswordHandler = async ({ input, ctx }: ChangePasswordOptions) => {
  ...
  // no-Postgres fork: `prisma.userPassword`/user reads THROW (no DB); auth lives in dibslist
  // Better Auth, not cal, so on the fork (`user.uuid` = the dibslist authUserId) this is N/A.
  if (user.uuid) {
