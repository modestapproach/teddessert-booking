...
export const resendVerifyEmail = async ({ input, ctx }: ResendEmailOptions) => {
  // no-Postgres fork: `prisma.secondaryEmail.findUnique` THROWS (no DB); email verification
  // lives in dibslist auth, so on the fork (`ctx.user.uuid` = the dibslist authUserId) N/A.
  if (ctx.user.uuid) {
