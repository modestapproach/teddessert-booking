...
export const outOfOfficeCreateOrUpdate = async ({ ctx, input }: TBookingRedirect) => {
  // no-Postgres fork: the `prisma.outOfOfficeEntry` upsert below THROWS (no DB); OOO is not
  // modeled on the dibslist booking backend, so on the fork (`ctx.user.uuid` = the dibslist
  // authUserId) no-op and return null.
  if (ctx.user.uuid) {
    return null;
  }
