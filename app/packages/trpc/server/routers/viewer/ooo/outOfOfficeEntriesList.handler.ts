...
export const outOfOfficeEntriesList = async ({ ctx, input }: GetOptions) => {
  // no-Postgres fork: `prisma.outOfOfficeEntry`/membership reads THROW (no DB), so on the
  // fork (`ctx.user.uuid` = the dibslist authUserId) return the empty OOO-entries table.
  if (ctx.user.uuid) {
    return {
      rows: [],
