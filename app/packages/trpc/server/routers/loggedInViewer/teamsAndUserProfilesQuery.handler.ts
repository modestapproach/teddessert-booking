...
export const teamsAndUserProfilesQuery = async ({ ctx, input }: TeamsAndUserProfileOptions) => {
  ...
  // no-Postgres fork: `prisma.user.findUnique` (teams/memberships) THROWS. On the fork
  // (`ctx.user.uuid` = the dibslist authUserId) there are no teams yet — return just the
  // personal profile so settings pages (webhooks/oauth target picker) still render.
  if (ctx.user.uuid) {
    return [
      {
        teamId: null,
        ...
        image: getUserAvatarUrl({ avatarUrl: null }),
        readOnly: false,
      },
    ];
  }
