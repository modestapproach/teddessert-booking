...
export const useTabsNavigations = ({
  ...
  const EventTypeTabs = useMemo(() => {
    ...
    // CV-7 — the co-host assignment tab. Upstream cal only showed this for TEAM
    // events; dibslist has no team concept, so the "add co-host by email" picker
    // (the "both founders free" headline) is always reachable. The tab itself
    // shows the collective toggle + email-add control regardless of `team`.
...
function getNavigation({
  ...
  const baseNavigation: VerticalTabItemProps[] = [
    ...
    {
      // BOOKING-LOTTERY — the app store is dead on this fork; the tab is the
      // "???" home for odd event-interaction modes (v1: the slot lottery).
      // Kept on the `apps` tabName key so existing URLs work.
      name: "???",
      href: `/event-types/${id}?tabName=apps`,
      icon: "grid-3x3",
      info: "odd event interactions",
      "data-testid": "apps",
    },
  ];
