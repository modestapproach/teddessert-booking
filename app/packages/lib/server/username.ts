...
export const isUsernameReservedDueToMigration = async (username: string) => {
  // CV — no-Postgres fork has no tempOrgRedirect table; this Prisma read throws and
  // 400s /api/username (the onboarding username availability check). No cal org-
  // migration redirects exist on the fork, so no username is reserved by migration.
  // (Booking-handle uniqueness is owned by Convex `calcomUserMap`, enforced on save.)
  if (process.env.NEXT_PUBLIC_CONVEX_URL) return false;
