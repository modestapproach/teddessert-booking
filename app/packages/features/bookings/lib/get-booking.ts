...
export const getBookingForReschedule = async (uid: string, userId?: number) => {
  // no-Postgres fork: bookings live in Convex, and every prisma read below THROWS —
  // 500ing the Booker page whenever `?rescheduleUid=` is present. Read the booking
  // from Convex (`getBookingByUid`, the same public read the confirmation page uses)
  // and map it to the GetBookingType shape the Booker needs for prefill + the
  // "former time" banner. No seats/teams on the fork; eventTypeId is left null so
  // processReschedule's cross-event-type redirect (a prisma read) never runs. The
  // prisma path is kept for a Postgres deploy.
  if (process.env.NEXT_PUBLIC_CONVEX_URL) {
    const { getConvex } = await import("@calcom/lib/server/convex");
    const { makeFunctionReference } = await import("convex/server");
    const getBookingByUidRef = makeFunctionReference<"query">(
      "scheduling/publicApi:getBookingByUid"
    );
    const cb = (await getConvex().query(getBookingByUidRef, { uid })) as {
      uid: string;
      startTime: number;
      endTime: number;
      status: string;
    ...
    const { BookingStatus } = await import("@calcom/prisma/enums");
    const statusMap: Record<string, (typeof BookingStatus)[keyof typeof BookingStatus]> = {
      accepted: BookingStatus.ACCEPTED,
      pending: BookingStatus.PENDING,
      cancelled: BookingStatus.CANCELLED,
      rescheduled: BookingStatus.CANCELLED,
    };
    const primary = cb.attendees[0];
    const mapped = {
      id: 0,
      uid: cb.uid,
      startTime: new Date(cb.startTime).toISOString(),
      endTime: new Date(cb.endTime).toISOString(),
      ...
      eventTypeId: null,
      status: statusMap[cb.status] ?? BookingStatus.ACCEPTED,
      userId: null,
      eventType: null,
      attendees: cb.attendees.map((a) => ({ name: a.name, email: a.email, bookingSeat: null })),
      user: null,
    };
    // GetBookingType is a Prisma-derived structural type; this boundary cast is the
    // established fork pattern (see getUserBooking.ts) — the Booker only reads the
    // fields mapped above.
    return mapped as unknown as Awaited<ReturnType<typeof getBooking>>;
  }
