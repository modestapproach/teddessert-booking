...
export class MembershipRepository {
  ...
  static async hasAnyAcceptedMembershipByUserId(userId: number) {
    // no-Postgres fork: `prisma.membership` reads THROW. The fork has no teams, so "has team
    // plan" is always false. The env signal (NEXT_PUBLIC_CONVEX_URL) gates the server-side RSC
    // callers (e.g. /settings/my-account/appearance via getCachedHasTeamPlan) that have no
    // `ctx.user.uuid` in scope. The Postgres path is left intact for a real deploy.
