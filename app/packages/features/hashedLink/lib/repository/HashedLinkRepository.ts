...
export class HashedLinkRepository {
  ...
  async findLinkWithEventTypeDetails(linkId: string) {
    // no-Postgres fork: `prisma.hashedLink` THROWS; private booking links aren't modeled → null.
  ...
  async findLinksWithEventTypeDetails(linkIds: string[]) {
    // no-Postgres fork: `prisma.hashedLink` THROWS; private booking links aren't modeled → [].
