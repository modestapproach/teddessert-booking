...
export class FeaturesRepository implements IFeaturesRepository {
  ...
  public async getAllFeatures() {
    // no-Postgres fork: `prisma.feature.findMany` THROWS. This fires on EVERY page
    // load via useFlags → trpc `features.map` (app-providers), so each render was a
    // silent server 500 + client retries. The fork doesn't use cal's feature-flag
    // table (its flags live in Convex); no rows = every flag reads disabled, which
    // is the intended fork default. The prisma path is kept for a Postgres deploy.
  ...
  public async getEnabledTeamFeatures(teamId: number) {
    // no-Postgres fork: `prisma.teamFeatures` THROWS, and the fork has no cal
    // teams — no rows (null) is the correct answer. Prisma path kept for Postgres.
    if (process.env.NEXT_PUBLIC_CONVEX_URL) {
      return null;
    }
  ...
  async checkIfUserHasFeature(userId: number, slug: string) {
    // CV-10: reached on the `/bookings/[status]` page SSR (the page builds
    // `new FeaturesRepository(prisma)` and calls this for "booking-audit"/"bookings-v3").
    // The `prisma.userFeatures.findFirst` THROWS on the no-Postgres fork and this method
    // RE-THROWS (catch → throw err below) → 500s the bookings dashboard. The no-Postgres
    // fork has no per-user feature-flag table; these flags default OFF, which is exactly the
    // page's fallback branch. On the fork (`NEXT_PUBLIC_CONVEX_URL` set — the env signal for
    // the anonymous/non-uuid call sites) we return `false` BEFORE the prisma read. The
  ...
  async checkIfTeamHasFeature(teamId: number, featureId: FeatureId): Promise<boolean> {
    // no-Postgres fork: hit by the event-type editor's Availability tab via the
    // public trpc `features.checkTeamFeature` on every render — prisma THROWS →
    // a silent 500 per render. The fork has no cal teams, so the flag is always
    // off. Prisma path kept for a Postgres deploy.
    if (process.env.NEXT_PUBLIC_CONVEX_URL) {
      return false;
    }
    try {
