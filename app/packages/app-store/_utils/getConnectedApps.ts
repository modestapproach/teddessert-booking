...
export async function getConnectedApps({
  ...
  user: Pick<User, "id" | "name" | "avatarUrl" | "email"> & { avatar?: string; uuid?: string };
  prisma: PrismaClient;
  input: InputSchema;
}) {
  const {
  ...
  // CV-10: this is reached on the CONNECTED-CALENDARS page load (`appsCaller.integrations`)
  // AND the event-type EDITOR's client `apps.integrations` query. The unconditional
  // `getUsersCredentialsIncludeServiceAccountKey` (raw `prisma.credential.findMany`) THROWS
  // on the no-Postgres fork → 500. The fork has no cal credential/app-store graph (calendar
  // connections are modeled separately via the CV-5 connectedCalendars surface), so the
  // owner has NO installed conferencing/app-store apps to enrich here. On the fork
  // (`user.uuid` = the dibslist authUserId, the no-Postgres signal) we skip the credential +
  // team prisma reads and run the rest of the pure registry-building logic over an EMPTY
  // credential set, yielding the correct "no installed apps" shape. The original prisma path
  // is LEFT INTACT for the `uuid`-absent case (a real Postgres cal.com deploy).
  const isConvexFork = !!user.uuid;
  let credentials = isConvexFork ? [] : await getUsersCredentialsIncludeServiceAccountKey(user);
  let userTeams: TeamQuery[] = [];

  if (!isConvexFork && (includeTeamInstalledApps || teamId)) {
