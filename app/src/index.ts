...
export class BookingApp extends Container {
  defaultPort = 3000;
  // THIS IS THE COST DIAL. Cloudflare bills memory per second only while the
  // instance is awake, so the monthly bill is (awake hours) x $0.036 for
  // standard-1. At "2h" a single 9am visit keeps 4 GiB warm until 11am and a
  // trickle of traffic through the day never lets it sleep — that is how the
  // previous setup reached ~$40/mo. "15m" comfortably covers one booking
  // session (browse slots -> confirm) while letting an idle day cost cents.
  // The trade: the first visitor after a quiet spell waits ~10-20s for the
  // container and Next to boot. Raise it if that latency matters more than
  // the money; lower it if it doesn't.
  sleepAfter = "15m";
  enableInternet = true;
