...
const UsernameTextfield = (props: ICustomUsernameProps & Partial<React.ComponentProps<typeof TextField>>) => {
  ...
  const updateUsernameMutation = trpc.viewer.me.updateProfile.useMutation({
    onSuccess: async () => {
      ...
      // CV: next-auth's session `update()` hits /api/auth/csrf, which 500s on the
      // no-Postgres fork (no next-auth backend). The username already persisted via
      // me.updateProfile → Convex and the session re-derives from Convex, so skip the
      // next-auth refresh on the fork.
