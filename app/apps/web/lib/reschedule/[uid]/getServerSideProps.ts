...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  // no-Postgres fork: every read below (seat lookup, prisma.booking, user enrich)
  // THROWS — the confirmation page's "Reschedule" link dead-ended in a 500. Read
  // the booking from Convex (`getBookingByUid`) and redirect to the Booker with
  // `?rescheduleUid=` exactly like stock cal. No seats/teams/prevention rules on
  // the fork (the backend re-validates everything at reschedule time anyway).
  if (process.env.NEXT_PUBLIC_CONVEX_URL) {
    const { uid: forkUid, allowRescheduleForCancelledBooking: forkAllowCancelled } =
    ...
    const cb = (await getConvex().query(getBookingByUidRef, { uid: forkUid })) as {
      status: string;
      eventType: { slug: string } | null;
      organizer: { username: string } | null;
    } | null;
    if (!cb || !cb.eventType?.slug || !cb.organizer?.username) {
      return { notFound: true } as const;
    }
    // A cancelled booking isn't reschedulable (unless explicitly allowed) — land
    // the booker on the booking page, mirroring cal's prevention redirect.
    if (cb.status === "cancelled" && !forkAllowCancelled) {
      return {
        redirect: { destination: `/booking/${forkUid}`, permanent: false },
      } as const;
    }
    const forkParams = new URLSearchParams();
    forkParams.set("rescheduleUid", forkUid);
    if (forkAllowCancelled) forkParams.set("allowRescheduleForCancelledBooking", "true");
    const forkCoep = context.query["flag.coep"];
    if (forkCoep) forkParams.set("flag.coep", forkCoep as string);
    return {
      redirect: {
        destination: `/${cb.organizer.username}/${cb.eventType.slug}?${forkParams.toString()}`,
        permanent: false,
      },
    } as const;
  }
