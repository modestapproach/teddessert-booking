...
const getTemporaryOrgRedirect = async ({
  ...
  // CV-11: the SSR PRE-HANDLER leaf. `handleOrgRedirect` runs at the TOP of the public
  // Booker `/[user]/[type]` SSR (both the single-user `getUserPageProps` and dynamic-group
  // `getDynamicGroupPageProps` branches) AND the `/[user]` profile SSR — BEFORE the
  // CV-2a/CV-10 Convex+synth reads. `isOrgContext` is always false single-user, so the
  // `if (!isOrgContext)` branch always reaches here and runs `prisma.tempOrgRedirect.findMany`,
  // which THROWS on the no-Postgres fork → 500 every public page before the good path runs.
  // The fork has NO org-rename history (single-user, no org migration), so there is never a
  // temp-org redirect: the cal-shaped "no redirect, continue SSR" value is `null` (matches the
  // declared `Promise<NextJsRedirect | null>` and the `if (redirect) return redirect;` call
  // sites, which fall through to `EventRepository.getPublicEvent`). This is an ANONYMOUS public
  // SSR path with no `ctx.user`, so we key off `NEXT_PUBLIC_CONVEX_URL` (the env signal CV-10
  // established for session-less paths). The prisma path is LEFT INTACT for a real Postgres deploy.
