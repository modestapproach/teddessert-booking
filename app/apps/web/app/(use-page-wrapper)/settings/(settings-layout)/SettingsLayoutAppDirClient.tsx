...
const useTabs = ({
  ...
  const processTabsMemod = useMemo(() => {
    ...
    return processedTabs.filter((tab) => {
      // no-Postgres fork: the Developer (webhooks/OAuth/API keys) + Admin sections are shimmed/dead
      // → hide them so the operator doesn't click into no-ops. (Organization is already hidden — no
      // org branding on the fork.)
      if (
        process.env.NEXT_PUBLIC_CONVEX_URL &&
        (tab.name === "developer" || adminRequiredKeys.includes(tab.name))
      )
        return false;
