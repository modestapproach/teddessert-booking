      ...
      <div className={classNames("text-subtle", className)}>
        {eventType.descriptionAsSafeHTML && (
          <div
            ...
            // biome-ignore lint/security/noDangerouslySetInnerHtml: descriptionAsSafeHTML is already sanitized server-side (markdownToSafeHTML)
            dangerouslySetInnerHTML={{
              // descriptionAsSafeHTML is ALREADY server-sanitized HTML. Render it
              // directly — re-running the server-only markdownToSafeHTML here both
              // double-processed already-HTML and pulled the costly markdown parser
              // (markdownIt + sanitize-html) into the CLIENT bundle, triggering the
              // "should not be imported on the client side" warning and contributing
              // to the React #419 Suspense SSR fallback noise. (cal.com PR #18195.)
