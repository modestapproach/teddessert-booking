...
// CV-7 — un-stubbed: the co-host-by-email assignment tab (the "both founders free"
// headline). dibslist has no team concept, so this is a minimal native control
// (built from @calcom/ui primitives) that drives the form `hosts[]` + schedulingType.
...
// BOOKING-LOTTERY — the Apps tab is repurposed as the "???" tab (interaction
// modes; v1 = the slot lottery). EventAppsTab stays on disk (frozen reference;
// it rendered an empty store on the fork — getConnectedApps is gated to []).
const EventInteractionsTab = dynamic(() =>
  import("./tabs/interactions/EventInteractionsTab").then((mod) => mod.EventInteractionsTab)
);
...
const EventTypeWeb = ({
  ...
  const tabMap = {
    ...
    // BOOKING-LOTTERY — "???" (interaction modes) lives on the old `apps` tab
    // key so existing ?tabName=apps URLs keep working.
    apps: <EventInteractionsTab eventTypeId={eventType.id} />,
