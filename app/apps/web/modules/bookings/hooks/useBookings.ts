...
export const useBookings = ({ event, hashedLink, bookingForm, metadata, isBookingDryRun }: IUseBookings) => {
  ...
  // BOOKING-LOTTERY — lottery-mode event types don't book on submit; the same
  // form data enters the slot's drawing and the booker lands on the countdown
  // page. `interactionMode` is an extra (non-cal) field the Convex public-event
  // adapter adds, hence the cast. A RESCHEDULE submit must NOT be hijacked into
  // a new drawing entry — it flows down the normal CV-3b reschedule path, where
  // the Convex backend rejects lottery-mode reschedules (lottery_only) so a
  // drawing winner can't slot-hop past other slots' drawings.
  const w2Mode = (
    event?.data as
      | { interactionMode?: "lottery" | "application" | "first_come" | "threshold" | "pair" | null }
      | null
      | undefined
  )?.interactionMode;
  const isLotteryEvent =
    !isRescheduling && (w2Mode === "lottery" || w2Mode === "application");

  const lotteryEnterMutation = useMutation({
    mutationFn: (data: unknown) =>
      post<unknown, { lotteryEntry: true; lotteryId: string }>("/api/lottery/enter", data),
    onSuccess: (entry) => {
      router.push(`/lottery/${entry.lotteryId}`);
    },
    onError: () => {
      if (bookerFormErrorRef?.current) {
        bookerFormErrorRef.current.scrollIntoView({ behavior: "smooth" });
  ...
  const handleBookEvent = useHandleBookEvent({
    ...
    // The lottery mutate takes the SAME BookingCreateBody wire shape (the enter
    // route re-maps it server-side); the cast only papers over react-query's
    // MutateOptions generic variance, not a data-shape difference.
    handleBooking: isLotteryEvent
      ? (lotteryEnterMutation.mutate as typeof createBookingMutation.mutate)
  ...
  const errors = {
    hasDataErrors: Boolean(
      createBookingMutation.isError ||
        ...
        lotteryEnterMutation.isError
    ),
    dataErrors:
      createBookingMutation.error || createRecurringBookingMutation.error || lotteryEnterMutation.error,
  };
  ...
  const loadingStates = {
    creatingBooking:
      ...
      lotteryEnterMutation.isPending ||
      lotteryEnterMutation.isSuccess,
