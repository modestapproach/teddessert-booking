...
// CV-1 — cal login is disabled: getServerSideProps now ALWAYS redirects
// (unauthenticated → dibslist login, authenticated → callback/home), so it no
// longer returns these props. inferSSRProps therefore resolves to `{}`. This
// view is dead (never rendered) but must still typecheck, so the props it once
// consumed are now declared optional locally.
...
export type PageProps = inferSSRProps<typeof getServerSideProps> & LegacyLoginProps;
