...
// CV-2a: `eventData` is now produced by `EventRepository.getPublicEvent` (Convex
// -backed) rather than the legacy Prisma `getPublicEvent`. Type it against the
// repository entry point so the SSR Props match the adapter's output shape.
type Props = {
  eventData: NonNullable<Awaited<ReturnType<typeof EventRepository.getPublicEvent>>>;
...
async function getDynamicGroupPageProps(context: GetServerSidePropsContext) {
  ...
  // CV-11: the DYNAMIC-GROUP leaf. The single-user `getUserPageProps:235` routes through the
  // CV-10-guarded `getUsersInOrgContext` (fork → synthesized `buildPublicForkUser`), but this
  // dynamic-group branch called `new UserRepository(prisma).findUsersByUsername(...)` DIRECTLY,
  // bypassing that guard → raw `prisma.user.findMany` (UserRepository.ts:175) THROWS on the
  // no-Postgres fork → 500s `/[u1]+[u2]/[type]`. Route it through the SAME guarded wrapper so
  // the fork synthesizes one cal-shaped user per username (event existence + meta still come
  // from the CV-2a `EventRepository.getPublicEvent` below). Prisma path intact off the fork.
...
async function getUserPageProps(context: GetServerSidePropsContext) {
  ...
  const props: Props = {
    ...
    bookingUid: bookingUid ? `${bookingUid}` : null,
    rescheduleUid: null,
    // CV-2a: the Convex public read carries no host/org identity, so there is no
    // org banner (documented field-gap). `eventData.owner` is always null.
    orgBannerUrl: null,
  };
  if (rescheduleUid) {
    const processRescheduleResult = await processReschedule({
