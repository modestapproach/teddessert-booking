// Lifted from cal.diy `packages/features/schedules/lib/slots.ts`
// (@180ede28, MIT). Adapted on lift:
//   - `@calcom/dayjs`                         -> `./dayjs`
//   - `DateRange` (@calcom date-ranges)       -> `./types`
//   - `IFromUser`/`IToUser`/`IOutOfOfficeData` -> `./types`
//   - `getTimeZone` (@calcom/lib/dayjs)       -> inlined (reads dayjs `$x.$timezone`)
//   - `withReporting` (Sentry wrapper)        -> stripped; `getSlots` exported directly
// The half-hour-offset-zone rounding (`.tz(timeZone)` BEFORE the
// `minute() % interval` check) and the slot-boundary snapping are preserved
// exactly.
import type { Dayjs } from "./dayjs";
import dayjs from "./dayjs";
import type { DateRange, IFromUser, IOutOfOfficeData, IToUser } from "./types";

// Inlined from `@calcom/lib/dayjs`: reads the timezone dayjs stashes on the
// instance after a `.tz()` call.
const getTimeZone = (date: Dayjs): string => (date as unknown as { $x: { $timezone: string } }).$x.$timezone;
...
export function buildSlotsWithDateRanges({
  dateRanges,
  frequency,
  eventLength,
  timeZone,
...
// Was wrapped in cal.diy with `withReporting(getSlots, "getSlots")` (a Sentry
// span wrapper that is a no-op pass-through when no DSN is configured). The
// wrapper is stripped here; the function body is unchanged.
