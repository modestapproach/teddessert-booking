// Lifted from cal.diy `packages/features/schedules/lib/date-ranges.ts`
// (@180ede28, MIT). Adapted on lift:
//   - `@calcom/dayjs`               -> `./dayjs`
//   - `Pick<Availability, ...>`     -> `WorkingHours` / `DateOverride` (./types)
//   - `IOutOfOfficeData` (@calcom)  -> `./types`
// The DST / offsetDiff logic and the `.tz(tz, true)` keepLocalTime override
// handling are preserved exactly.
import type { Dayjs } from "./dayjs";
import dayjs from "./dayjs";
import type { DateOverride, DateRange, IOutOfOfficeData, WorkingHours } from "./types";

export type { DateOverride, DateRange, WorkingHours } from "./types";
